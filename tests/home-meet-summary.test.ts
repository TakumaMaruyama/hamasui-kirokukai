...
describe("buildHomeMeetComparisonCards", () => {
  it("builds two cards from the latest three distinct swimming months and ignores other programs", () => {
    ...
    expect(cards?.[0]?.currentMeet?.id).toBe("2026年3月");
    expect(cards?.[0]?.currentMeet?.participantCount).toBe(2);
  ...
  it("compares only the latest three distinct months and ignores older months", () => {
    const cards = buildHomeMeetComparisonCards([
      buildMeet({
        id: "march",
        program: "swimming",
      ...
      buildMeet({
        ...
        program: "swimming",
        title: "2025年5月",
        heldOn: "2025-05-01T00:00:00.000Z",
  ...
  it("returns waiting-older-month when there are only two distinct months", () => {
    const cards = buildHomeMeetComparisonCards([
      buildMeet({
        id: "march",
        program: "swimming",
  ...
  it("returns waiting-next-meet and unavailable when there is only one distinct month", () => {
    const cards = buildHomeMeetComparisonCards([
      buildMeet({
        id: "march-sun",
        program: "swimming",
