# 仕様

## CSV列定義（program列なし）
- `meet_title`: 記録会名称
- `held_on`: 開催日（YYYY-MM-DD）
- `full_name`: 氏名（検索対象。フルネーム完全一致）
- `grade`: 学年（数値）
- `gender`: `male` / `female` / `other`
- `event_title`: 種目名（例: 50m自由形）
- `style`: 泳法（例: free, fly）
- `distance_m`: 距離（数値）
- `lane`: レーン番号（任意）
- `time_text`: 記録（例: 0:45.23, 45.23）

CSVの program 列は存在しない。取込時の画面/APIパスで program を固定する。

- 小学校水泳授業（`school`）では、`time_text` の `a`（大文字・全角も可）を欠席として扱う。
- 小学校の出席者の `event_title` は自由入力とし、入力された名称を記録証へ表示する。欠席者（`time_text = a`）は空欄を許可する。
- 小学校の `meet_title` は学校名として扱う。簡易形式では1校1ファイルとし、CSVファイル名から学校名を補完する。

### 名簿CSV（簡易形式）の取り込み
- 画面で `年 / 月 / 曜日` を指定してからプレビューする。
- 名簿CSVのように `meet_title` / `held_on` が無い場合は、指定した `年 / 月 / 曜日` から内部値を補完する。
- 補完時:
  - `meet_title`: `YYYY年M月X曜`
  - `held_on`: `YYYY-MM-01`
- 小学校では種目名から距離を判定できない場合も取り込み、内部の分類用距離は `0` とする。種目名の表示値は変更しない。

## 画面一覧
- `/` トップ（検索フォーム）
- `/athletes/[id]` 個人ページ（記録推移・種目別順位）
- `/athletes/history` 子ども統合ページ（氏名+性別単位、学校年度ごとの履歴）
- `/admin` 管理者ログイン
- `/admin/import/swimming` スイミング取込
- `/admin/import/school` 学校委託取込
- `/admin/import/challenge` チャレンジコース取込
- `/admin/publish` 公開期間・お知らせ表示管理
- `/admin/docs/swimming` スイミングPDF生成
- `/admin/docs/school` 学校委託PDF生成
- `/admin/docs/challenge` チャレンジコースランキングPDF生成
- `/admin/logs` 検索ログ閲覧

## API一覧
- `POST /api/search` 検索（レート制限・検索ログ保存）
- `GET /api/athletes/:id` 個人表示
- `POST /api/admin/login`
- `POST /api/admin/import/swimming/preview`
- `POST /api/admin/import/swimming/confirm`
- `POST /api/admin/import/school/preview`
- `POST /api/admin/import/school/confirm`
- `POST /api/admin/import/challenge/preview`
- `POST /api/admin/import/challenge/confirm`
- `POST /api/admin/docs/swimming/records`
- `POST /api/admin/docs/swimming/certificates`
- `POST /api/admin/docs/swimming/rankings`
- `POST /api/admin/docs/school/records`
- `POST /api/admin/docs/challenge/rankings`
- `GET /api/admin/logs`

### 検索APIの同意仕様
- `POST /api/search` は毎回同意必須。
- リクエストは `{ fullName, consentAccepted, consentVersion }`。
- `consentAccepted` が `true` でない場合は検索できない。
- `consentVersion` がサーバー定義と不一致の場合は再同意を求める。
- 検索結果は学年単位ではなく、`fullName + gender` で集約した子ども単位で返す。
- 検索結果要素は `{ fullName, gender, grades }`（`grades` は昇順の学年一覧）。

### PDF出力APIの条件指定
- `POST /api/admin/docs/*` はJSONで条件指定できる（任意）。
- `year` + `month` + `weekday` で「指定年月の曜日ごと」に出力可能。
- `year` + `month` + `fullName` で「特定の子どもの指定年月」を出力可能（`weekday` 併用可）。
- スイミング1位賞状PDF（`/api/admin/docs/swimming/certificates`）は年月指定必須で、曜日指定は受け付けない。
- スイミング1位賞状PDFは対象月内の `種目 × 学年 × 男女` ごとの1位を出力し、同タイ1位は全員掲載する。
- 1位賞状の氏名指定は、月全体の順位を確定した後に適用する。指定した子どもが1位でない場合は賞状を出力しない。
- 一般コースの作成画面では登録済みの対象年月を共通選択し、記録証・1位賞状・歴代1位記録一覧は対象確認後にPDFをダウンロードする。
- `GET /api/admin/docs/swimming/months` は登録済み年月を新しい順に返す。初期選択は最新月とする。
- `POST /api/admin/docs/swimming/preview` は `kind`（`records` / `certificates` / `historical-firsts`）、年月、および対応する曜日・氏名条件を受け取り、`filename`・`counts`・`items` を返す。空対象は件数0、生成・保存は行わない。年月指定は必須。
- 対象確認とPDF生成は同じ集計処理を使用し、生成時には最新の記録を読み直す。年月変更で全確認結果、曜日・氏名変更で該当書類の確認結果を破棄する。
- 歴代1位は指定月末までの記録を対象とし、種目の表記差は月次と同じ比較キーで統合する。同タイ、記録年月、対象月に新しく歴代1位へ到達した記録のNEW表示を維持する。曜日・氏名条件は受け付けない。
- 月次ランキングPDF（`/api/admin/docs/swimming/rankings` と `/api/admin/docs/challenge/rankings`）は年月指定必須。
- 月次ランキングPDFはA4で種目ごとに男女左右2列表示、各学年の1〜3位を出力する（同順位は全員掲載）。
- スイミング月次ランキングPDFの学年は小6までを対象とし、種目ごとに最小学年〜最大学年を連続表示して、欠番学年も空欄で出力する（中1以降は出力しない）。
- チャレンジ月次ランキングPDFの学年は全学年を対象とし、種目ごとに最小学年〜最大学年を連続表示して、欠番学年も空欄で出力する。
- 月次ランキングPDFでは `gender = other` は出力対象外とする。
- チャレンジCSV取り込みは学年 `1..15`（年少〜高校3年生）のみを有効とし、範囲外はスキップして件数を通知する。

## DBスキーマ要約
- athletes: 氏名・学年・性別
- meets: (program, held_on, title) ユニーク（program: `swimming` / `school` / `challenge`）
- events: 種目情報（学年/性別別）
- results: タイムと順位（DENSE_RANK）
- generated_docs: 生成物の保存キー
- search_logs: 検索ログ（検索名、IP、User-Agent、同意バージョン）
- publish_windows: ユーザー画面に表示する公開期間（開始日/終了日）とお知らせ

## ログ/レート制限仕様
- 検索はフルネーム完全一致のみ。
- 検索APIはIP単位でレート制限をかける（1分あたり10回想定）。
- 検索時に毎回同意チェックを必須にする。
- 検索実行時に `search_logs` に保存。
- `search_logs` には `consentVersion` も保存する。
- 公開期限はユーザー画面の案内表示専用（自動非公開は行わない）。

## 子ども統合ページの表示仕様
- `/athletes/history` はクエリ `fullName` と `gender` を受け取り、同一キー（氏名+性別）で複数学年の履歴を統合表示する。
- 年度表示は学校年度（4月〜翌3月、UTC基準）を使用する。
- 同姓同名・同性別の別人をDB上で完全判別できないため、統合表示される可能性がある旨を画面で案内する。

## PDF生成/ZIP
- スイミング記録証と1位賞状は、条件に一致した全員分をそれぞれ1つのPDFにまとめて生成する。
- ランキングなど単一PDF以外の生成物は必要に応じてZIPにまとめる。
- スイミング記録証・1位賞状・歴代1位記録一覧はダウンロードのみとし、ファイル保存と生成履歴登録は行わない。過去の保存ファイルと生成履歴は削除しない。月次ランキング・小学校・チャレンジの保存方式は維持する。
- 一般コース記録証は現在の掲載順で1枚最大4件。5件目以降は掲載せず、追加ページは作成しない。対象確認にも実際に掲載する4件までを表示する。
- 歴代1位記録一覧はZIPに包まずPDFを直接返す。ダウンロード名は `Content-Disposition` のUTF-8ファイル名を優先する。
- 小学校記録証は学校ごとに1つのPDFへまとめる。タイム `a` の児童は種目・記録表を表示しない専用レイアウトにし、「水泳学習、よくがんばりました。」の文面を使う。来年についての文言は入れない。
- 小学校の記録は順位計算・順位表示の対象外とする。
- PDF生成ライブラリで字形が壊れる「𠮷」は、CSV・DBの値を変えず、PDF表示時だけ「吉」へ置換する。
- 小学校記録証には「学校委託コース」を表示せず、最下部に `開催年月 YYYY年M月` を表示する。
- 小学校記録証が4種目を超える場合は、5種目目以降を次ページへ続けて全件出力する。
